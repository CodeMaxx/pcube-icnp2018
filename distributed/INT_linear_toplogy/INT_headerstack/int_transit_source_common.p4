...
control Int_metadata_insert(inout headers hdr, in int_metadata_t int_metadata,inout standard_metadata_t standard_metadata)
{
        ...
        action int_set_header_4() { //ingress_timestamp
            ...
            hdr.int_ingress_tstamp[0].ingress_tstamp = (bit<32>) standard_metadata.enq_timestamp;

        }
        action int_set_header_5() { //egress_timestamp
            //hdr.int_egress_tstamp.setValid();
            hdr.int_egress_tstamp.push_front(1);
            hdr.int_egress_tstamp[0].egress_tstamp =(bit<32>) standard_metadata.enq_timestamp + (bit<32>) standard_metadata.deq_timedelta;
        ...
        apply{
            if(hdr.int_header.isValid()) {
                if(hdr.int_header.remaining_hop_cnt != 0 && hdr.int_header.e == 0) {
                    int_hop_cnt_decrement(); // dont add apply() if directly calling action
                    int_prep.apply();
